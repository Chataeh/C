//배열의 출력과 통계처리를 위한 함수들 메인함수에서 헤더파일을 통한 사용
#include <stdio.h>
#include<limits.h>
#include<math.h> 
#include<time.h>
...
void printBigArraySample(int* array, int size, int line_size, int sample_lines) //배열출력
{
	int num = 0;
	...
	if (num < (size - line_size * sample_lines))
		num = size - line_size * sample_lines; //num= (전체크기)-(출력할려는 총 배열의 수)
...
void fprintBigArraySample(FILE* fout, int* array, int size, int line_size, int sample_lines)// 배열을 파일에 출력
{
	int num = 0;
...
void getArrayStatistics(int* array, int size)//통계정보계산 후 출력
{
	...
	int sum = 0;
	double avg = 0;
	double diff, var, sq_diff_sum = 0, std;
